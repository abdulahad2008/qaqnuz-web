...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: `${t("automation247.title")} — Qaqnuz`,
    description: t("automation247.description"),
  };
}

const METRIC_ICONS = [Clock, MessageSquare, Star, Zap];
const METRIC_COLORS = ["hsl(174 72% 56%)", "hsl(var(--accent))", "#4ade80", "#c084fc"];

export default async function TwentyFourSevenCaseStudy() {
  const t = await getTranslations("caseStudiesPage");
  const data = t.raw("automation247") as {
    tags: string[];
    title: string;
    description: string;
